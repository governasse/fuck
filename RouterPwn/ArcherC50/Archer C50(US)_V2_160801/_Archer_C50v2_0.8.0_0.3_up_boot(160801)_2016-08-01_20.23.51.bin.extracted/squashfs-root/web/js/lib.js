...
*/	
	        ...
	        switch (type) {
	            ...
	            case "name":
	                ...
	                htmlStr += "<span>" + (obj.content ? obj.content : '') + "</span>";
                        ...
                        htmlStr += "<h5 class=\"title name-title\">" + obj.title + "<span class=\"help-arrow\">arrow</span></h5>";
	...
	*/
	baseIndexOf: function(array, value, fromIndex) {
 	...
 	jumpToOtherPageFromTips: function(id, page) {
        $($('#' + id + '_tips div.error-tips-content').css({
            cursor: 'default'
        }).find('a')).click(function() {
            var goTo = function(p) {
                var li = $('#menuTree').find('a[url="' + p + '"]').parent('li');
