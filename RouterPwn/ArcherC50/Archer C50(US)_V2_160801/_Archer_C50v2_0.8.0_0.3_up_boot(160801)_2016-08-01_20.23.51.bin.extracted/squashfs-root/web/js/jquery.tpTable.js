(function($) {
    ...
    TPTable.prototype = {
        ...
        addIconClick: function() { 
            ...
            self.$table.next().show().css('top', '0').height(self.$table.find('.editor-container').first().position().top + self.$table.position().top);

            tr.show().height();

            self.$table.next().next().show().css('bottom', '0').css('top', 'auto').height(self.$table.height() - self.$table.find('.editor-container').first().position().top - tr.height() + turnpage);
...
jQuery.extend({
    ...
    initTableBody: function(table, array) {
        ...
        var tr = table.find("tbody tr:not(.nd)");
        tr.remove();
        for (var i = 0; i < array.length; i++) {
            var tr = $("<tr></tr>");
    ...
    allCheckboxSelected: function(tableLabel) {
        var td = tableLabel.find("tbody tr td[class='table-content']");
        var checkall = true;
        td.find("input[type='checkbox']").each(function(index) {
            if ($(this).next("label").siblings('.checkbox-checkbox').prop('data-checked') === false) {
                checkall = false;
            }
        });
