...
jQuery.extend({
    ...
    alertAsnyc: function() {
        ...
        tmp.find("span.text").css({
            ...
            "max-width": 700
        }).html(m_str.errno + ":"+ errno + "<br>" + str);
        $.setCentralPosition(tmp);
        $.lock();

        var showCover = function() {
            var tmp = $('body').find('#mask-alert-asnyc').length > 0 ? $('body').find('#mask-alert-asnyc') : $("<div id=\"mask-alert-asnyc\" class=\"mask\"><div id=\"cover-loading\"></div></div>").appendTo($("body"));
            tmp.fadeIn(200, function() {
